...
finalsets = [ ###these are from TM in MCF7, can use to compare. 
            #cell cycle
...
for c in ['TM','TW','MW']:
    ...
    #hallmarks gene sets
    c = fdrs[(fdrs.index.str.contains(sets))&((cup[cup<0.01].any(axis=1))|(cdown[cdown<0.01].any(axis=1)))]
    ...
    #drug targets
    tar = fdrs[fdrs.index.str.contains(targets)&((cup[cup<0.01].any(axis=1))|(cdown[cdown<0.01].any(axis=1)))]

    final = pd.concat([final,sig,tar,c]).drop_duplicates()

final.to_csv('hit_geneset_enrichment_fdr_3combos.tsv',sep='\t')
finallogged = -np.log10(final)
finallogged.to_csv('hit_geneset_enrichment_fdr_log_3combos.tsv',sep='\t')
...
finallogged = finallogged[['down_'+tx+'_'+time for tx in newtxs for time in times] +\
    ['up_'+tx+'_'+time for tx in newtxs for time in times]].drop(
                          'regulation of neuron death (GO:1901214)') #not relevant to this cell type
#seaborn heatmap
...
hm=sns.clustermap(finallogged,col_cluster=False,cmap='hot_r',vmax=6,
...
#
#finallogged[tmcols].to_csv('TM_hits.tsv',sep='\t')
#
...
#
#finallogged[mwcols].to_csv('MW_hits.tsv',sep='\t')
#
#twcols = [
#        'down_T_3', 'down_T_6', 'down_T_9', 'down_T_12', 'down_T_24',
#        'down_W_3', 'down_W_6', 'down_W_9', 'down_W_12', 'down_W_24',
...
#
#finallogged[twcols].to_csv('TW_hits.tsv',sep='\t')
