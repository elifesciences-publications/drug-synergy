...
ddply(clusters_annots_cycling, .(Cluster), function(x) x[which.min(x$padj),] )
...
#!
TWdf <- total_filtered_interaction_TW[, c(2,21:26)]
rownames(TWdf) <- TMdf$GeneSymbol
#TWdf$GeneSymbol <- NULL
write.csv( TWdf, 'TW_specific_interaction_genes.csv', row.names = T, col.names = T, quote = F)
...
# data <- plotCounts(cycl_expr_deseq, geneid, 
#                    intgroup=c("Time_unordered","Treatment"), returnData=TRUE)
# #!merge with voom log2cpm 
# data <- merge(data, melt(v[geneid, ]$E), by.x = 0, by.y = 'Var2')
# #reset to numeric for smoothing
# data$Time <- as.numeric(as.character(data$Time))
# #plot cycle
# p1 <- ggplot(data, aes(x = Time, y = value, color = Treatment, fill = Treatment)) + geom_point() + 
# 							stat_smooth(se = F, method = 'loess') +  ggtitle( paste(geneid, ': cluster ', clusterid, sep = '') ) + 
# #							scale_y_continuous(breaks = round(seq(min(data$count), max(data$count), by = floor(max(data$count)/5)),1)) #+ 
# 							#scale_y_continuous(breaks = round(seq(min(data$value), max(data$value), by = floor(max(data$value)/5)),1)) #+ 
# 							scale_x_continuous( breaks = c(0, 3, 6, 9, 12, 36) )
# print(p1)
# p[[j]] <<- p1
# ###
# 								})
...
#!
#!normalized to DMSO, reformatted
tophits <- ddply(total_filtered_interaction_TM, .(Cluster), function(x) x[which.max(x$baseMean),] )
...
colnames(data2)[1] <- 'DMSO_count'
data <- merge( data, data2[, c('replicate', 'DMSO_count')], by = 'replicate')
data$DMSO_relative_count <- log2(data$count/data$DMSO_count)
#reset to numeric for smoothing
data$Time <- as.numeric(as.character(data$Time))
#plot cycle
p1 <- ggplot(data[data$Treatment %in% c('T', 'M', 'TM'),], aes(x = Time, y = DMSO_relative_count, color = Treatment, fill = Treatment)) + geom_point() + 
							...
							#scale_y_continuous(breaks = round(seq(min(data$DMSO_relative_count), max(data$DMSO_relative_count), 
							#																	by = (max(data$DMSO_relative_count)/5)),1)) + 
							xlab( 'Time (hours)') + ylab( 'log2(cpm) - log2(cpm_DMSO)' )		+		
						    scale_y_continuous(breaks = round(seq(-3, 5, by = 1),1)) 	+									 #+ 
							scale_x_continuous( breaks = c(0, 3, 6, 9, 12, 24) ) + ylim(-3, 5.1) #+ xlim(0, 24)
print(p1)
p[[j]] <<- p1
###
								})

pdf('Fig_S3b_mod.pdf', height = 8, width = 12)
multiplot( p[[1]],p[[4]], p[[2]],p[[5]],p[[3]], cols = 3)
dev.off()
